...
def get_i2_filter(object_type="Host", slack_message=""):
    ...
    if slack_message.strip() is not "":
        # clean up the slack message from autogenerated links
        logging.info("Slack Message: %s" % slack_message)
        if '|' in slack_message:
            slack_message = slack_message.replace('<', '').replace('>', '').split('|')[1]
    ...
    logging.info("Filter states: %s" % filter_states)
    logging.info("Filter names: %s" % filter_options)
    logging.info("Filter errors: %s" % filter_error)
    logging.info("Filter object type: %s" % object_type)
